        ...
        <div className="container mx-auto flex justify-between items-center  py-4 px-4 md:px-6">
          ...
          <nav className="hidden  md:flex items-center space-x-8 ">
            {["Home", "About us", "Our Staff", "Blogs", "Feedback", "FAQ's", "Services", "Contact us"].map((item) => (
              <a key={item} href={`/${item.toLowerCase().replace(" ", "")}`} className="text-[15px] font-medium hover:text-[#1d5c5c]">
                {item}
              </a>
            ))}
          </nav>
          ...
          {/* Mobile Menu Button */}
          <a   href="/" className=" ml-2 mr-2 md:hidden ">
            <img src={logo} alt="City Dental Care" className="h-auto w-[90px]" />
          </a>
